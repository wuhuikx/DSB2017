...
def ResBody(net, from_layer, index, input_channel, output_channel, stride = 1):
  ...
  if stride != 1 or input_channel != output_channel:
    ...
    net[index + '_shortcut_bn'] = L.BatchNorm(
                          ...
                          filler=dict(type = 'uniform', \
                          min = 0, max = 1), include = dict(phase = 0), **en_kwargs)
    net['test_' + index + '_shortcut_bn'] = L.BatchNorm(\
                          net[index + '_shortcut_conv'], use_global_stats = True, \
                          include = dict(phase = 1), name = index + '_shortcut_bn', \
                          top = index + '_shortcut_bn', ntop = 0, **en_kwargs)
    net['scale_' + index + '_shortcut_bn'] = L.Scale(net[index + '_shortcut_bn'], \
                                                     bias_term = True)
...
net.data, net.coord = L.Input(shape=[ \
                     dict(dim = im_size), \
                     dict(dim = coord_size)], ntop = 2)
...
net['rpn_output_relu'] = L.ReLU(net['rpn_output'], in_place = False)#, **en_kwargs)

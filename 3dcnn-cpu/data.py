...
class DataBowl3Detector(object):
    def __init__(self, data_dir, split_path, phase = 'train',split_comber=None):
        assert(phase == 'train' or phase == 'val' or phase == 'test')    
        self.phase = phase
        self.stride = config['stride']       
        self.isScale = config['aug_scale']
        self.r_rand = config['r_rand']
    ...
    def getdata(self, idx, split = None):
        ...
        if self.phase != 'test':
            ...
            sample = (sample.astype(np.float32)-128) / 128 #128)/128
            
            return sample, label, coord, patient_id
        else:
            ...
            #print '----------------before split ----------------  '
            #print('before_data0 = ', imgs.shape)
            #print('before_bboxes0 = ', bboxes.shape)
            #print('before_coord0 = ', coord.shape)
            ...
            imgs = (imgs.astype(np.float32)-128) / 128 #128)/128
            #print '----------------nzhw ----------------  ', nzhw
            #print('data0 = ', imgs.shape)
            #print('bboxes0 = ', bboxes.shape)
            #print('coord0 = ', coord2.shape)
            return imgs, bboxes, coord2, np.array(nzhw)
        
        
    def get_minibatch(self, idcs, phase = 'TRAIN', split = None):
        if phase != 'TEST':
            data1 = []
            target1= []
            coord1 = []
            ...
            for idx in idcs:
                data, target, coord, patient_id = self.getdata(idx, split)
                data1.append(data)
                target1.append(target)
                coord1.append(coord)   
              
                
            return [np.array(data1), np.array(target1), np.array(coord1)]
    
        else:

            imgs, bboxes, coord, nzhw = self.getdata(idcs[0], split)               
            return [np.array(imgs), np.array(bboxes), np.array(coord), np.array(nzhw)]

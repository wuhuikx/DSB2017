...
class GetPBB(object):
    ...
    def __call__(self, output,thresh = -3, ismask=False):
        ...
        #print('oz.reshape =', oz.reshape((-1, 1, 1, 1)))
...
#         coordlist: n x k x 3 x 24 x 24 x 24
        ...
        # using NoduleNet to compute score of nodules
        noduleFeat,nodulePred = self.NoduleNet(xlist,coordlist)
        
        get_pbb = GetPBB()
        pbbs = get_pbb(nodulePred)
        
        print('noduleFeat=',noduleFeat.size())  # [1, 128, 24, 24, 24]
        print('nodulePred=',nodulePred.size()) # [1, 24, 24, 24, 3, 5]
        #print('----------step3----------')
        #nodulePred = nodulePred.contiguous().view(corrdsize[0],corrdsize[1],-1)  
        print('nodulePred_size=',nodulePred.size())
        
        print(nodulePred[0,0,0,0,:,:])
        
        print('xsize=',xsize)
        pbbs = self.__get_proposals(pbbs)  #nodulePred

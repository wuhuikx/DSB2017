...
class Loss(nn.Module):
    ...
    def forward(self, output, labels, train = True):
        #print('batch_size=',labels.size(0))
        #print('forward-output.size=',output.size())
        ...
        if len(pos_output)>0:
            ...
            #print('neg_prob=',neg_prob)
            #print('neg_labels=',neg_labels+1)
            #print('self.classify_loss(neg_prob, neg_labels + 1)=',self.classify_loss(neg_prob, neg_labels + 1))
            
            pos_correct = (pos_prob.data >= 0.5).sum()
            pos_total = len(pos_prob)

            #print('----------------------------')
            #print('regress_losses=',regress_losses)
...
class GetPBB(object):
    ...
    def __call__(self, output,thresh = -3, ismask=False):
        ...
        print('oz.reshape((-1, 1, 1, 1)) =', oz.reshape((-1, 1, 1, 1)))
        print('oh.reshape((-1, 1, 1, 1)) =', oh.reshape((-1, 1, 1, 1)))
        print('ow.reshape((-1, 1, 1, 1)) =', ow.reshape((-1, 1, 1, 1)))
        print('anchors=', anchors)
        print('anchors.reshape((1, 1, 1, -1))=',anchors.reshape((1, 1, 1, -1)))

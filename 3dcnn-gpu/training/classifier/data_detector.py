...
class DataBowl3Detector(Dataset):
    def __init__(self, split, config, phase = 'train',split_comber=None):
        ...
        for idx in idcs:
            ...
            if np.all(l==0):  # put negative samples as []
                l=np.array([])
                count_neg +=1
            labels.append(l) # just put the positive samples
        count_pos = len(labels) - count_neg    
        ...
        #print('self.label_mapping=',self.label_mapping)

    ...
    for im in FROCImList_rand:
        ...
        if not candidatesExists:
            ...
            candidates = np.concatenate(
                (candidates, scanToCandidatesDict[im]), axis=1)
...
def computeFROC_bootstrap(FROCGTList, FROCProbList, FPDivisorList, FROCImList, excludeList, numberOfBootstrapSamples=1000, confidence=0.95):
    ...
    set1 = np.concatenate(
        ([FROCGTList], [FROCProbList], [excludeList]), axis=0)
    ...
    for i in range(numberOfBootstrapSamples):
        ...
        fps, sens, thresholds = computeFROC(btpsamp[0, :],
                                            btpsamp[1, :],
                                            len(FROCImList_np),
                                            btpsamp[2, :])
    ...
    interp_sens = np.zeros((numberOfBootstrapSamples,
                            len(all_fps)),
                           dtype='float32')
    ...
    # compute mean and CI
    sens_mean, sens_lb, sens_up = compute_mean_ci(interp_sens,
...
def evaluateCAD(seriesUIDs, results_filename, outputDir, allNodules, CADSystemName, maxNumberOfCADMarks=-1,
    ...
    # open output files
    nodNoCandFile = open(os.path.join(
        outputDir, "nodulesWithoutCandidate_%s.txt" % CADSystemName), 'w')
    ...
    for seriesuid in seriesUIDs:
        ...
        for noduleAnnot in noduleAnnots:
            ...
            for key, candidate in candidates.iteritems():
                ...
                dist = math.pow(x - x2, 2.) + math.pow(y -
                                                       y2, 2.) + math.pow(z - z2, 2.)
                        ...
                        if bOtherNodulesAsIrrelevant:  # delete marks on excluded nodules so they don't count as false positives
                            if key in candidates2.keys():
                                ...
                                ignoredCADMarksList.append("%s,%s,%s,%s,%s,%s,%.9f" % (
                                    seriesuid, -1, candidate.coordX, candidate.coordY, candidate.coordZ, str(candidate.id), float(candidate.CADprobability)))
                                del candidates2[key]
            ...
            if noduleAnnot.state == "Included":
                ...
                if found == True:
                    ...
                    FROCtoNoduleMap.append("%s,%s,%s,%s,%s,%.9f,%s,%.9f" % (seriesuid, noduleAnnot.id, noduleAnnot.coordX, noduleAnnot.coordY, noduleAnnot.coordZ, float(
                        noduleAnnot.diameter_mm), str(candidate.id), float(candidate.CADprobability)))
                    candTPs += 1
                else:
                    candFNs += 1
                    ...
                    FROCtoNoduleMap.append("%s,%s,%s,%s,%s,%.9f,%s,%s" % (seriesuid, noduleAnnot.id, noduleAnnot.coordX,
                                                                          noduleAnnot.coordY, noduleAnnot.coordZ, float(noduleAnnot.diameter_mm), int(-1), "NA"))
                    nodNoCandFile.write("%s,%s,%s,%s,%s,%.9f,%s\n" % (seriesuid, noduleAnnot.id, noduleAnnot.coordX,
                                                                      noduleAnnot.coordY, noduleAnnot.coordZ, float(noduleAnnot.diameter_mm), str(-1)))
    ...
    if not (len(FROCGTList) == len(FROCProbList)
    ...
    if int(totalNumberOfNodules) > 0:
        ...
        ax = plt.gca()
        clr = 'b'
        plt.plot(fps_itp, sens_itp, color=clr, label="%s" %
                 CADSystemName, lw=2)
        if performBootstrapping:
        ...
        plt.savefig(os.path.join(outputDir, "froc_%s.png" %
                                 CADSystemName), bbox_inches=0, dpi=300)
...
parser = argparse.ArgumentParser(description='subset evaluation')
parser.add_argument('--subset', default='0', type=str, metavar='N',
                    help='the idx of the subset')
parser.add_argument('--inoutpath', default='0', type=str, metavar='N',
                    help='the idx of the subset')
...
if __name__ == '__main__':
    ...
    idx = args.subset
    pathname = 'luna_anno_wh/froc/split/{0}'.format(idx)
    #pathname = 'luna_anno_wh/froc/total'
    annotations_filename = os.path.join(pathname, 'annos_new.csv')
    annotations_excluded_filename = os.path.join(pathname, 'annos_e_new.csv')
    seriesuids_filename = os.path.join(pathname, 'seriesuids.csv')  #'seriesuids_new.csv'
    
    #results_filename = '/home/wuhui/3dcnn/result-wh/nodnet/input128/2017_09_14/test_val.csv'
    #outputDir = '/home/wuhui/3dcnn/result-wh/nodnet/input128/2017_09_14/evaluation'
    #results_filename = '/home/wuhui/3dcnn/result-wh/nodnet/cpu/input64/test_val.csv'
    #outputDir = '/home/wuhui/3dcnn/result-wh/nodnet/cpu/input64/evaluation'
    
    #results_filename = '/home/wuhui/3dcnn/result-wh/nodnet/input128/2017_09_14/{0}/test_val.csv'.format(idx)
    #outputDir = '/home/wuhui/3dcnn/result-wh/nodnet/input128/2017_09_14/{0}/evaluation'.format(idx)
    #results_filename = '/home/wuhui/3dcnn/result-wh/nodnet/2017_10_10/{0}/pred_val.csv'.format(idx)
    #outputDir = '/home/wuhui/3dcnn/result-wh/nodnet/2017_10_10/{0}/evaluation'.format(idx)
    results_filename = '/home/wuhui/3dcnn/result-wh/nodnet/input192/{0}/test_val.csv'.format(idx)
    outputDir = '/home/wuhui/3dcnn/result-wh/nodnet/input192/{0}/evaluation'.format(idx)
    #results_filename = '/home/wuhui/3dcnn/result-wh/nodnet/cpu/input64/{0}/test_val.csv'.format(idx)
    #outputDir = '/home/wuhui/3dcnn/result-wh/nodnet/cpu/input64/{0}/evaluation'.format(idx)
    ...
    fp = open(os.path.join(outputDir, 'froc_test_val_bootstrapping.csv'), 'r')
    ...
    res = np.array(res)
    mean_res = np.mean(np.array(res[:, 1]).astype(float))
    print('res =', res)
    print('mean =', mean_res)
    
    fp = open(os.path.join(outputDir, 'mean.txt'), 'w')
    fp.write(res)
    fp.write('mean = ' + str(mean_res))
    fp.close()

    '''annotations_filename = os.path.join('./luna_anno', 'luna_val_annos.csv')
    annotations_excluded_filename = os.path.join(
        './luna_anno', 'luna_val_annos_e.csv')
    seriesuids_filename = os.path.join('./luna_anno', 'luna_uids.csv')
    results_filename = './zhai/submission/pred.csv'
    outputDir = './zhai/evaluation'
    # execute only if run as a script
    
    
    
    (annotations_filename, annotations_excluded_filename,
                        seriesuids_filename, results_filename, outputDir)
    print "Finished!"'''

...
class DataBowl3Detector(Dataset):
    def __init__(self, data_dir, split_path, config, phase = 'train',split_comber=None):
        ...
        #print('filenames[0] = ', self.filenames[0])        
        #print('lunanames = ', len(self.lunanames))
        #print('lunanames[0] = ', self.lunanames[0])
    ...
    def __getitem__(self, idx,split=None):
        ...
        if self.phase != 'test':
            ...
            sample = (sample.astype(np.float32)-128 * self.scale) / (128 * self.scale)            
           
            return torch.from_numpy(sample), torch.from_numpy(label), coord, patient_id
        else:
            ...
            imgs = (imgs.astype(np.float32)-128 * self.scale) / (128 * self.scale)   
...
class Crop(object):
    ...
    def __call__(self, imgs, target, bboxes,isScale=False,isRand=False):
        ...
        if isScale:
            radiusLim = [8. * self.scale ,120. * self.scale]
            scaleLim = [0.75,1.25]

...
def hard_mining(neg_output, neg_labels, num_hard):    
    #print('neg_output1=', neg_output)
    _, idcs = torch.topk(neg_output, min(num_hard, len(neg_output)))
    neg_output = torch.index_select(neg_output, 0, idcs)
    neg_labels = torch.index_select(neg_labels, 0, idcs)
    
    #print('neg_output2=', neg_output)
...
class Loss(nn.Module):
    ...
    def forward(self, output, labels, train = True):
        ...
        #print('output = {0} \t labels = {1}'.format(output.size(), labels.size()))
        ...
        if len(pos_output)>0:
            ...
            #print('c_pos_loss = {0} \t c_neg_loss = {1}'.format(self.classify_loss( \
            #pos_prob, pos_labels[:, 0]), self.classify_loss(neg_prob, neg_labels + 1)))
            
            
            pos_correct = (pos_prob.data >= 0.5).sum()
            pos_total = len(pos_prob)
            #print('pos_labels = {0} \t neg_labels = {1}'.format(pos_labels[:, 0], neg_labels))

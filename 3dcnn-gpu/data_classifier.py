...
class DataBowl3Classifier(Dataset):
	def __init__(self, split, config, phase='train'):
		...
		self.filenames = [os.path.join(
			datadir, '%s_clean.npy' % idx.split('-')[0]) for idx in idcs]
		if self.phase != 'test':
...
#             if idx.startswith()
	...
	def __getitem__(self, idx, split=None):
		...
		if self.random_sample and self.phase == 'train':
			...
			chosenid = conf_list.argsort()[::-1][:topk]
		croplist = np.zeros([topk, 1, self.crop_size[0],
							 self.crop_size[1], self.crop_size[2]]).astype('float32')
		coordlist = np.zeros([topk, 3, self.crop_size[0] / self.stride, self.crop_size[1] /
							  self.stride, self.crop_size[2] / self.stride]).astype('float32')
...
class simpleCrop():
	...
	def __call__(self, imgs, target):
		...
		imgs = np.pad(imgs, pad, 'constant',
					  constant_values=self.filling_value)
		crop = imgs[:, start[0]:start[0] + crop_size[0], start[1]
			:start[1] + crop_size[1], start[2]:start[2] + crop_size[2]]
...
def augment(sample, coord, ifflip=True, ifrotate=True, ifswap=True, filling_value=0):
	...
	if ifrotate:
		...
		sample = rotate(sample, angle1, axes=(2, 3),
						reshape=False, cval=filling_value)
	...
	if ifflip:
		flipid = np.array(
			[np.random.randint(2), np.random.randint(2), np.random.randint(2)]) * 2 - 1

...
class DataBowl3Detector(Dataset):
    def __init__(self, split, config, phase='train', split_comber=None):
        ...
        if self.channel == 2:
            self.filenames = [os.path.join(
                data_dir, '%s_merge.npy' % idx) for idx in idcs]
        elif self.channel == 1:
            if 'cleanimg' in config and config['cleanimg']:
                self.filenames = [os.path.join(
                    data_dir, '%s_clean.npy' % idx) for idx in idcs]
            else:
                self.filenames = [os.path.join(
                    data_dir, '%s_img.npy' % idx) for idx in idcs]
        self.kagglenames = [f for f in self.filenames if len(
            f.split('/')[-1].split('_')[0]) > 20]
        self.lunanames = [f for f in self.filenames if len(
            f.split('/')[-1].split('_')[0]) < 20]
        ...
        for idx in idcs:
            if config['luna_raw'] == True:
                try:
                    ...
                    print('idx =', idx)
                    print('path = ', os.path.join(data_dir, '%s_label.npy' % idx))
                    
                    l = np.load(os.path.join(
                        data_dir, '%s_label_raw.npy' % idx))
                except:
    ...
    def __getitem__(self, idx, split=None):
        ...
        if self.phase != 'test':
            if not isRandomImg:
                ...
                sample, target, bboxes, coord = self.crop(
                    imgs, bbox[1:], bboxes, isScale, isRandom)
                ...
                isScale = self.augtype['scale'] and (self.phase == 'train')
                sample, target, bboxes, coord = self.crop(
                    imgs, [], bboxes, isScale=False, isRand=True)
        ...
        #	label[label==-1]=0
            ...
            imgs = np.pad(imgs, [[0, 0], [0, pz - nz], [0, ph - nh],
                                 [0, pw - nw]], 'constant', constant_values=self.pad_value)
            xx, yy, zz = np.meshgrid(np.linspace(-0.5, 0.5, imgs.shape[1] / self.stride),
                                     np.linspace(-0.5, 0.5,
                                                 imgs.shape[2] / self.stride),
                                     np.linspace(-0.5, 0.5, imgs.shape[3] / self.stride), indexing='ij')
            coord = np.concatenate(
                [xx[np.newaxis, ...], yy[np.newaxis, ...], zz[np.newaxis, :]], 0).astype('float32')
...
def augment(sample, target, bboxes, coord, ifflip=True, ifrotate=True, ifswap=True):
    ...
    if ifflip:
        ...
        flipid = np.array(
            [1, np.random.randint(2), np.random.randint(2)]) * 2 - 1
    ...
    def __call__(self, imgs, target, bboxes, isScale=False, isRand=False):
        if isScale:
            ...
            scale = np.random.rand() * \
                (scaleRange[1] - scaleRange[0]) + scaleRange[0]
            crop_size = (np.array(self.crop_size).astype(
        ...
        for i in range(3):
            if not isRand:
                ...
                s = np.max([imgs.shape[i + 1] - crop_size[i] / 2,
                            imgs.shape[i + 1] / 2 + bound_size])
                e = np.min([crop_size[i] / 2,
                            imgs.shape[i + 1] / 2 - bound_size])
            ...
            if s > e:
                ...
                start.append(int(target[i]) - crop_size[i] / 2 +
                             np.random.randint(-bound_size / 2, bound_size / 2))

        normstart = np.array(start).astype('float32') / \
            np.array(imgs.shape[1:]) - 0.5
        normsize = np.array(crop_size).astype(
            'float32') / np.array(imgs.shape[1:])
        xx, yy, zz = np.meshgrid(np.linspace(normstart[0], normstart[0] + normsize[0], self.crop_size[0] / self.stride),
                                 np.linspace(
            normstart[1], normstart[1] + normsize[1], self.crop_size[1] / self.stride),
...
class LabelMapping(object):
    ...
    def __call__(self, input_size, target, bboxes):
        ...
        oz = np.arange(offset, offset + stride *
                       (output_size[0] - 1) + 1, stride)
        oh = np.arange(offset, offset + stride *
                       (output_size[1] - 1) + 1, stride)
        ow = np.arange(offset, offset + stride *
                       (output_size[2] - 1) + 1, stride)

        for bbox in bboxes:
            for i, anchor in enumerate(anchors):
                iz, ih, iw = select_samples(bbox, anchor, th_neg, oz, oh, ow)
                label[iz, ih, iw, i, 0] = 1
                label[:, :, :, i, 0] = binary_dilation(label[:, :, :, i, 0].astype(
                    'bool'), structure=struct, iterations=1).astype('float32')
        ...
        if self.phase == 'train' and self.num_neg > 0:
            ...
            neg_idcs = random.sample(
                range(len(neg_z)), min(num_neg, len(neg_z)))
        ...
        if len(iz) == 0:
            ...
            for i in range(3):
                pos.append(
                    max(0, int(np.round((target[i] - offset) / stride))))

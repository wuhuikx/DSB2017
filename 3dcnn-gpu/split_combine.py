...
class SplitComb():
        ...
        for iz in range(nz):
            for ih in range(nh):
                for iw in range(nw):
                    ...
                    print('np.newaxis=',np.newaxis,';','sz=',sz,';','ez=',ez,';','sh=',sh,';','eh=',eh,';')
    ...
    def combine(self, output, nzhw=None, side_len=None, stride=None, margin=None):
        ...
        print('nz*side_len=',nz*side_len,';','shape[3]=',splits[0].shape[3],';','shape[4]=',splits[0].shape[4])
        ...
        for iz in range(nz):
            for ih in range(nh):
                for iw in range(nw):
                    ...
                    split = splits[idx][int(margin):int(margin + side_len),
                                        int(margin):int(margin + side_len), int(margin):int(margin + side_len)]
